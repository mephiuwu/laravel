...
<div class=" flex justify-center p-4">
    {{--  <div class="flex justify-center w-full mt-4 pb-2">
        <h1 class="font-bold text-gray-700 text-5xl">Contáctanos</h1>
      </div>
 --}}
    <div class="bg-gray-400 text-gray-500 rounded-3xl shadow-xl flex justify-center overflow-hidden w-full sm:flex-col">

        <form action="{{route('contacto.email')}}" enctype="multipart/form-data" method="post"
            class="md:flex  w-full h-full">
            @csrf
            <div class="hidden md:block md:w-1/2 bg-white  py-10 px-10">
 ...
 --}} <img src="{{asset('public/img/contact.svg')}}" style="height: 100%" class="h-full" alt="Img Contacto"
                    width="100%">
            </div>
            <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                ...
                <div class="flex flex-col justify-start space-y-5  h-full   ">
                    <div class="flex flex-wrap -mx-3">
                        <div class="w-full md:w-1/2 px-3 mb-5">
                            <span class="uppercase text-sm text-gray-600 font-bold">Nombre*</span>
                            <input placeholder="Ingrese su nombre" name="nombre" id="nombre" value="{{old('nombre')}}"
                        ...
                        <div class="w-full md:w-1/2 px-3 mb-5">
                            ...
                            <input placeholder="Ingrese su nombre" name="email" id="email" value="{{old('email')}}"
                                class="w-full bg-white text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
                                type="text">
                            @error('email')
                            <b class="text-red-500 mb-0">{{$message}}</b>
                            @enderror
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3">
                        <div class="w-full md:w-1/2 px-3 mb-5">
                            <span class="uppercase text-sm text-gray-600 font-bold">Región*</span>
                            <select
                                class="w-full bg-white text-gray-900 mt-2 p-3 py-4 rounded-lg focus:outline-none focus:shadow-outline"
                        ...
                        <div class="w-full md:w-1/2 px-3 mb-5">
                            <span class="uppercase text-sm text-gray-600 font-bold">Comuna*</span>
                            <select
                                class="w-full bg-white text-gray-900 mt-2 p-3 py-4 rounded-lg focus:outline-none focus:shadow-outline"
                    ...
                    <div class="flex flex-wrap  -mx-3">
                        <div class="w-full md:w-1/2  px-3 mb-5">
                            <span class="uppercase text-sm text-gray-600 font-bold">Dirección*</span>
                            <input placeholder="Ingrese su dirección" value="{{old('direccion')}}" name="direccion" id="direccion"
                                class="w-full bg-white text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
                                type="text">
                            @error('direccion')
                            <b class="text-red-500">{{$message}}</b>
                        ...
                        <div class="w-full md:w-1/2  px-3 mb-5">
                            ...
                            <input class="w-full bg-white text-gray-900 mt-2 p-3 rounded-lg 
                                            focus:outline-none focus:shadow-outline" name="telefono" id="telefono"
                                value="{{old('telefono')}}" type="tel" placeholder="Ingrese su teléfono">
                            @error('telefono')
                            <b class="text-red-500">{{$message}}</b>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-wrap  -mx-3">
                        <div class="w-full md:w-1/2  px-3 mb-5">
                            <span class="uppercase text-sm text-gray-600 font-bold">Asunto*</span>
                            <select
                                class="w-full bg-white text-gray-900 mt-2 p-3 py-4 rounded-lg focus:outline-none focus:shadow-outline"
                                name="asunto" id="asunto">
                                <option value="">Seleccione una opción</option>
                                @foreach ($asuntos as $asunto)
                                <option value="{{$asunto->id}}" {{old('asunto') == $asunto->id ? 'selected' : ''}}>{{$asunto->asun_nombre}}
                                </option>
                                @endforeach
                            </select>
                            @error('asunto')
                        ...
                        <div class="w-full md:w-1/2  px-3 mb-5">
                            <span class="uppercase text-sm text-gray-600 font-bold">Archivo</span>
                            <input type="file" name="documento" id="documento"
                                class="w-full bg-white text-gray-900 mt-2 p-2 rounded-lg focus:outline-none focus:shadow-outline">
                            @error('documento')
                            <b class="text-red-500 flex flex-wrap">{{$message}}</b>
                            @enderror
                        </div>
                    </div>
                    <div class="flex -mx-3">
                       <div class="w-full px-3 mb-5">
                        ...
                        <textarea name="mensaje" id="mensaje"
                            class="w-full h-32 bg-white   text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline" style="min-height: 100px; max-height:250px;"></textarea>
                        @error('mensaje')
                        <b class="text-red-500">{{$message}}</b>
                        @enderror
                       </div>
                    ...
                    <div class="flex -mx-3">
                            <div class="w-full px-3 mb-5 lg:px-40">
                            <button type="submit"
                            class="uppercase w-full mb-5 text-sm font-bold tracking-wide bg-indigo-500 text-gray-100 p-3 rounded-lg  focus:outline-none focus:shadow-outline">
                            Enviar Mensaje
                            </button>
                        </div>
                     </div>
...
{{-- <div
                class="mt-4 grid gap-8  lg:grid-cols-2 md:grid-cols-1 pb-10 mx-auto  text-gray-900 rounded-lg ">
                 
                <div class="img_contact flex flex-nowrap flex-col justify-center items-center" style="height: 100%; width: 100%; min-width: 100%;">
                  <img src="{{asset('public/img/contact.svg')}}" class="flex justify-center items-center"
style="height: 100%; width: 100%; object-fit:contain; object-position:center;" alt="Img Contacto" >
</div>
<div class="form ">
    ...
    <div class="mt-8 flex flex-col">
        <span class="uppercase text-sm text-gray-600 font-bold">Región*</span>
        <select
            class="w-full bg-gray-300 text-gray-900 mt-2 p-3 py-4 rounded-lg focus:outline-none focus:shadow-outline"
            name="region" id="region">
            <option value="">Seleccione una opción</option>
            @foreach($regiones as $region)
            <option value="{{$region->id}}" {{old('region') == $region->id ? 'selected' : ''}}>
...
<div class="form ">
    <div class="flex flex-col">
        <span class="uppercase text-sm text-gray-600 font-bold">Teléfono*</span>
        <input class="w-full bg-gray-300 text-gray-900 mt-2 p-3 rounded-lg 
                        focus:outline-none focus:shadow-outline" name="telefono" id="telefono"
            value="{{old('telefono')}}" type="tel" placeholder="Ingrese su teléfono">
        @error('telefono')
        <b class="text-red-500">{{$message}}</b>
    ...
    <div class="mt-8 flex flex-col">
        <span class="uppercase text-sm text-gray-600 font-bold">Archivo</span>
        <input type="file" name="documento" id="documento"
            class="w-full bg-gray-300 text-gray-900 mt-2 p-2 rounded-lg focus:outline-none focus:shadow-outline">
        @error('documento')
        <b class="text-red-500 flex flex-wrap">{{$message}}</b>
        @enderror
    </div>
...
<script>
    var message = @json(session('contact_success'));
